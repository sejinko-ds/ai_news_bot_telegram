...
def get_fetcher(site: SiteConfig) -> BaseFetcher:
    """Create a fetcher instance for a site config."""
    ...
    # Generic fallback - use RSSFetcher as default
    return RSSFetcher(name=site.key, url=site.url, max_articles=site.max_articles)
...
async def run_pipeline(
    ...
    """
    Main fetch-deduplicate-notify pipeline.
    ...
    recipient_configs = [r for r in config.recipients if r.enabled]
    if recipients and "all" not in recipients:
        recipient_configs = [r for r in recipient_configs if str(r.chat_id) in recipients]
    ...
    # Send to recipients
    if all_new_articles and recipient_configs:
        for source_name, articles in all_new_articles.items():
            for recipient in recipient_configs:
                try:
                    sent = await notifier.send_articles(
                        recipient.chat_id, articles, source_name
                    )
                    summary["sent"] += sent
                except Exception as e:
                    error_msg = f"Send error to {recipient.name}: {e}"
                    logger.error(error_msg)
                    summary["errors"].append(error_msg)

            # Mark as sent after successful delivery to all recipients
            for article in articles:
                storage.mark_sent(
                    article.fingerprint, article.source, article.url, article.title
                )
